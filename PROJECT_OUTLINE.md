...
- Train Model：用已审阅标注训练当前标签的帧级分类器（删除的标注区域自动作为硬负样本参与训练）
- Auto-label Unreviewed：用训练好的模型自动标注未审阅区域
- Clear Negatives：一键清除当前标签的全部硬负样本（带确认对话框和数量提示）
- Annotation Legend：颜色图例

### 硬负样本反馈机制（非 phase 类型专用）
- 用户删除标注时，自动将删除区域记录为当前标签的硬负样本
- 训练时，硬负样本区域强制标记为负类，增强模型对误标区域的辨识能力
- ML 工具栏实时显示当前标签的负样本数量（tooltip）
- 支持一键清除，方便用户在标注策略调整后重置负样本
