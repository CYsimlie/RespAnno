...
class TestNegSampleManager:
    ...
    def test_add_empty_label_returns_none(self):
        ...
        assert mgr.add("  ", 1.0, 2.0) is None  # str("  ") is truthy but .strip() would be empty
        # Actually "" is empty string, "  " is truthy in Python.
        # The check is `if not label: return None`.
        # Let's test both cases.
        mgr2 = NegSampleManager()
        assert mgr2.add("", 1.0, 2.0) is None   # empty string → None
        assert mgr2.add("x", 1.0, 2.0) is not None  # non-empty → works
    ...
    def test_remove_nonexistent_does_not_crash(self):
        mgr = NegSampleManager()
        mgr.remove("Wheeze", 999)  # should not raise

    def test_remove_nonexistent_label_does_not_crash(self):
        mgr = NegSampleManager()
        mgr.remove("nonexistent", 1)  # should not raise
    ...
    def test_clear_single_label(self):
        ...
        mgr.clear("Wheeze")
        assert mgr.count("Wheeze") == 0
        assert mgr.count("Crackles") == 1  # other label untouched
    ...
    def test_clear_all_resets_id_counter(self):
        ...
        # After clear_all, id counter resets to 0, so new id should be 1
        assert new_item[2] == 1
    ...
    def test_to_dict_is_live_reference(self):
        ...
        # Mutations through the dict affect the manager
        d["Wheeze"].clear()
        assert mgr.count("Wheeze") == 0
