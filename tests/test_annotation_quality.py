...
# ---------------------------------------------------------------------------
# normalize_annotation invariants
# ---------------------------------------------------------------------------
...
class TestRoundtrip:
    def test_csv_roundtrip_preserves_all_fields(self):
        original = [
            ...
            (1.8, 2.4, "Crackles", "ml"),
            (3.0, 4.1, "Inspiration", "auto_accepted"),
        ]
    ...
    def test_json_roundtrip_preserves_all_fields(self):
        original = [
            (0.5, 1.2, "Wheeze", "manual"),
            (1.8, 2.4, "Crackles", "auto_edited"),
            (3.0, 4.1, "Inspiration", "merged"),
        ]
        anns = [normalize_annotation(a) for a in original]
        with tempfile.NamedTemporaryFile(
...
# ---------------------------------------------------------------------------
# Deduplication / overlap logic (used by classifier & phase_model)
# ---------------------------------------------------------------------------
...
class TestOverlapRatio:
    ...
    def test_partial_overlap(self):
        # new=(0,2), old=(1,3) → overlap=(1,2)=1, ratio=1/2=0.5
    ...
    def test_threshold_50_percent(self):
        ...
        # This is what classifier.py and phase_model.py implement
        assert overlap_ratio((0, 2), (1, 3)) >= 0.5  # 50% overlap → skip
        assert overlap_ratio((0, 2), (1.5, 3)) < 0.5  # 25% overlap → keep
...
class TestFixtureFiles:
    ...
    def test_bad_rows_tolerated(self):
        ...
        rows = read_annotations(path)
        # Any valid rows extracted; no crash on bad rows
