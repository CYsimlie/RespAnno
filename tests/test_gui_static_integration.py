...
def test_gui_calls_read_annotations():
    ...
    assert "read_annotations" in src, "1.6.6.py must call read_annotations"


def test_gui_calls_write_annotations():
    src = _tokenized_source(GUI_FILE)
    assert "write_annotations" in src, "1.6.6.py must call write_annotations"
...
def test_gui_calls_preprocessing_functions():
    ...
    assert not missing, f"Missing preprocessing calls: {missing}"


# ---------------------------------------------------------------------------
# 4. Stage 3: spectrogram is still connected
# ---------------------------------------------------------------------------
...
def test_gui_calls_spectrogram_functions():
    ...
    missing = [fn for fn in required if fn not in src]
    assert not missing, f"Missing spectrogram calls: {missing}"


# ---------------------------------------------------------------------------
# 5. Stage 4: features is connected
# ---------------------------------------------------------------------------
...
def test_gui_imports_features():
    ...
    target = "respanno.dsp.features"
    found = any(
        target in imp or imp.startswith("respanno.dsp.features")
        for imp in imports
    )
    assert found, f"1.6.6.py must import from {target}"
...
def test_gui_calls_features_functions():
    ...
    missing = [fn for fn in required if fn not in src]
    assert not missing, f"Missing features calls: {missing}"


# ---------------------------------------------------------------------------
# 6. hsmm is NOT connected yet
# ---------------------------------------------------------------------------

def test_no_early_module_leak():
    """1.6.6.py must NOT import from hsmm yet."""
    tree = _parse_ast(GUI_FILE)
    forbidden = {"respanno.ml.hsmm"}
    for node in ast.walk(tree):
...
ALLOWED_RESPANNO = {"annotation_io", "preprocessing", "spectrogram", "features"}
