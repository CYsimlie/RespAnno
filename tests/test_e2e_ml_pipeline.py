...
class TestE2EWheezeClassifier:
    ...
    def test_pipeline_runs_without_error(self):
        audio, sr, anns = generate_wheeze_episode(
            duration=5.0, wheeze_start=1.0, wheeze_dur=2.5, seed=42
        )
        ...
        # may or may not generate predictions depending on threshold
    ...
    def test_model_trains_on_correct_label_only(self):
        ...
        # Train a classifier for a label that only appears as Crackles
        ok = train_event_model(viewer, "Rhonchi", random_state=42)
        # No Rhonchi annotations → should fail
        assert ok is False
    ...
    def test_full_phase_pipeline(self):
        ...
        # mock phase_model QMessageBox too
...
class TestFrameLabelE2E:
    ...
    def test_labels_integrate_with_real_features(self):
        audio, sr, anns = generate_wheeze_episode(
            duration=4.0, wheeze_start=1.0, wheeze_dur=1.0, seed=42
        )
